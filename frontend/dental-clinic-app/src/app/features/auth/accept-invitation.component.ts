...
interface InvitationPreview { status: number; email?: string; role?: string; expiresAt?: string; }
...
@Component({
  ...
  template: `
    <section class="auth-card">
      ...
      @if (loading()) { <div class="loading" role="status">{{ text('Validating invitation…', 'جارٍ التحقق من الدعوة…') }}</div> }
      @else if (success()) {
        <div class="alert success" role="status">{{ text('Account activated. Redirecting to login…', 'تم تفعيل الحساب. جارٍ الانتقال لتسجيل الدخول…') }}</div>
        ...
        <form [formGroup]="form" (ngSubmit)="accept()">
          ...
          <label>{{ text('Confirm password', 'تأكيد كلمة المرور') }}
            <input type="password" formControlName="confirmPassword" autocomplete="new-password">
          </label>
          <button class="primary" type="submit" [disabled]="submitting() || form.invalid">
            {{ submitting() ? text('Activating…', 'جارٍ التفعيل…') : text('Activate account', 'تفعيل الحساب') }}
          </button>
        </form>
      } @else {
...
export class AcceptInvitationComponent {
  ...
  constructor() {
    this.http.post<InvitationPreview>('/api/auth/invitations/inspect', { token: this.token }).subscribe({
      next: value => { this.preview.set(value); this.loading.set(false); },
      error: () => { this.preview.set({ status: 0 }); this.loading.set(false); }
    });
  }
  ...
  accept(): void {
    ...
    this.http.post('/api/auth/invitations/accept', { token: this.token, password, confirmPassword }).subscribe({
      next: () => {
