...
@Component({
  ...
  template: `
    <div class="reports-page">
      <div class="header">
        <h2>Treatment Analytics / تقرير العلاجات</h2>
      </div>
      ...
      <div *ngIf="loading" class="state-msg">Loading treatment data...</div>
      <div *ngIf="error" class="state-msg error">{{ error }}</div>

      <div *ngIf="!loading && data" class="content">
        <div class="kpi-grid">
          <app-report-kpi-card
            title="Total Treatments / إجمالي العلاجات"
          ...
          ></app-report-kpi-card>
          <app-report-kpi-card
            title="Treatment Revenue / إجمالي الإيراد"
        ...
        <div class="charts-grid">
          <app-svg-bar-chart
            title="Treatments By Type / حسب النوع"
            [items]="typeBarItems"
          ></app-svg-bar-chart>
          <app-svg-bar-chart
            title="Treatments By Doctor / حسب الطبيب"
...
export class TreatmentReportComponent implements OnInit {
  ...
  loadData(): void {
    this.loading = true;
    this.api.getTreatments(this.filter).subscribe({
      next: (res) => {
        this.data = res;
        this.loading = false;
      },
      error: (err) => {
        this.error = err?.error?.title || 'Failed to load treatment report.';
        this.loading = false;
      },
    });
  }
  ...
  downloadCsv(): void {
    this.api.downloadCsv('treatments', this.filter).subscribe((blob) => {
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'treatment-report.csv';
      a.click();
    });
  }
  ...
  get typeBarItems(): BarChartItem[] {
    if (!this.data) return [];
    return this.data.byType.map((t) => ({ label: t.typeName, value: t.count, color: '#10b981' }));
  }
