...
@Component({
  ...
  template: `
    <div class="reports-page">
      <div class="header">
        <h2>Revenue Analysis / تحليل الإيرادات</h2>
      </div>
      ...
      <div *ngIf="loading" class="state-msg">Loading revenue data...</div>
      ...
      <div *ngIf="!loading && data" class="content">
        ...
        <div class="charts-grid">
          <app-svg-line-chart
            title="Revenue Over Time / الإيرادات عبر الزمن"
            [items]="lineItems"
          ></app-svg-line-chart>

          <app-svg-bar-chart
            title="Revenue By Doctor / الإيرادات حسب الطبيب"
            [items]="doctorBarItems"
          ></app-svg-bar-chart>

          <app-svg-bar-chart
            title="Revenue By Treatment Type / الإيرادات حسب نوع العلاج"
            [items]="treatmentBarItems"
          ></app-svg-bar-chart>

          <app-svg-bar-chart
            title="Revenue By Category / الإيرادات حسب الفئة"
...
export class RevenueReportComponent implements OnInit {
  ...
  loadData(): void {
    this.loading = true;
    this.api.getRevenue(this.filter).subscribe({
      next: (res) => {
        this.data = res;
        this.loading = false;
      },
      error: (err) => {
        this.error = err?.error?.title || 'Failed to load revenue report.';
        this.loading = false;
      },
    });
  }
  ...
  downloadCsv(): void {
    this.api.downloadCsv('revenue', this.filter).subscribe((blob) => {
  ...
  get lineItems(): LineChartItem[] {
    if (!this.data) return [];
    return this.data.byPeriod.map((p) => ({ label: p.period, value: p.amount }));
  }

  get doctorBarItems(): BarChartItem[] {
    if (!this.data) return [];
    return this.data.byDoctor.map((d) => ({ label: d.doctorName, value: d.revenue }));
  }

  get treatmentBarItems(): BarChartItem[] {
    if (!this.data) return [];
    return this.data.byTreatment.map((t) => ({ label: t.treatmentType, value: t.revenue }));
  }

  get categoryBarItems(): BarChartItem[] {
    if (!this.data) return [];
