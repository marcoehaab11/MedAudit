...
@Component({
  ...
  template: `
    <section class="page-head">
      <div>
        <p class="eyebrow">{{ text('Clinic team', 'فريق العيادة') }}</p>
        <h1>{{ text('Users', 'المستخدمون') }}</h1>
      </div>
      <button class="primary" type="button" (click)="showInvite.set(!showInvite())">
        {{ text('Invite user', 'دعوة مستخدم') }}
      </button>
    </section>
    @if (success()) {
      ...
      <section class="panel invite-panel">
        ...
        <form [formGroup]="inviteForm" (ngSubmit)="invite()" class="form-grid">
          ...
          <div class="form-actions">
            <button class="primary" [disabled]="inviteForm.invalid || saving()">
              {{
                saving()
                  ? text('Sending…', 'جارٍ الإرسال…')
                  : text('Send invitation', 'إرسال الدعوة')
              }}
            </button>
          </div>
        </form>
    ...
    <form [formGroup]="filters" (ngSubmit)="load(1)" class="filters panel">
      <input
        ...
        [placeholder]="text('Search name or email', 'ابحث بالاسم أو البريد')"
      />
      <select formControlName="roleId">
        <option value="">{{ text('All roles', 'كل الأدوار') }}</option>
        @for (role of roles(); track role.id) {
          <option [value]="role.id">{{ role.name }}</option>
        }
      </select>
    ...
    <section class="panel table-panel">
      @if (loading()) {
        <div class="loading" role="status">
          {{ text('Loading users…', 'جارٍ تحميل المستخدمين…') }}
        </div>
      } @else if (!result()?.items?.length) {
        <div class="empty">
          <strong>{{ text('No users found', 'لا يوجد مستخدمون') }}</strong>
          <p>
            {{
              text(
                'Invite a team member or adjust the filters.',
                'ادعُ عضواً أو عدّل عوامل التصفية.'
              )
            }}
          </p>
        </div>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th>
                  <span class="sr-only">{{ text('Actions', 'الإجراءات') }}</span>
                </th>
              </tr>
            </thead>
            <tbody>
              @for (user of result()!.items; track user.id) {
                <tr>
                  ...
                  <td>
                    <a [routerLink]="['/users', user.id]">{{ text('Manage', 'إدارة') }}</a>
                  </td>
                </tr>
              }
            </tbody>
    ...
    this.api
      ...
      .subscribe({
        next: () => {
          ...
          this.success.set(this.text('Invitation sent.', 'تم إرسال الدعوة.'));
          this.load(1);
        },
        error: () => {
          this.saving.set(false);
          this.error.set(
            this.text(
              'Invitation could not be sent. Check the form and permissions.',
              'تعذر إرسال الدعوة. تحقق من البيانات والصلاحيات.',
            ),
          );
        },
      });
