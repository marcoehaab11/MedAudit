...
public sealed class PatientAllergy : TenantOwnedEntity
{
    private PatientAllergy() { }
    public PatientAllergy(Guid tenantId, Guid patientId, string name, string? notes, DateTimeOffset createdAt)
    {
        SetOwner(tenantId, patientId); Name = PatientField.Required(name, nameof(name), 200);
        Notes = PatientField.Optional(notes, nameof(notes), 1000); CreatedAt = createdAt; UpdatedAt = createdAt;
    }
